...
class Notifications {
	...
	public function __construct() {
		...
		add_action( 'admin_notices', [ $this, 'wc_version_requirement_notice' ] );
	...
	public function php_requirement_notice() {
		if ( ! rtwpvg()->is_valid_php_version() ) {
			...
			$message = wp_kses( __( "It's required to use latest version of PHP to use <strong>Variation Images Gallery for WooCommerce</strong>.", 'woo-product-variation-gallery' ), [ 'strong' => [] ] );

			printf( '<div class="%1$s"><p>%2$s <a target="_blank" href="%3$s">%4$s</a></p></div>', $class, $message, $link, $text ); // phpcs:ignore
		}
	}
	...
	public function wc_requirement_notice() {
		if ( ! rtwpvg()->is_wc_active() ) {
			...
			$message = wp_kses( __( '<strong>Variation Images Gallery for WooCommerce</strong> is an add-on of ', 'woo-product-variation-gallery' ), [ 'strong' => [] ] );

			printf( '<div class="%1$s"><p>%2$s <a class="thickbox open-plugin-details-modal" href="%3$s"><strong>%4$s</strong></a></p></div>', $class, $message, $link, $text ); //phpcs:ignore
		}
	}
	...
	public function wc_version_requirement_notice() {
		if ( rtwpvg()->is_wc_active() && ! rtwpvg()->is_valid_wc_version() ) {
			...
			printf( '<div class="%1$s"><p><strong>%2$s</strong></p></div>', $class, $message ); //phpcs:ignore 
		}
	}
