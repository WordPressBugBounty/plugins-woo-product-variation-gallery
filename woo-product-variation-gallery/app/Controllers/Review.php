...
class Review {
	...
	public function __construct() {
		$current      = time();
		$black_friday = mktime(0, 0, 0, 11, 17, 2022) <= $current && $current <= mktime(0, 0, 0, 1, 15, 2023);
		if ( ! $black_friday ) {
			register_activation_hook( RTWPVG_PLUGIN_FILE, [ __CLASS__, 'rtwpvg_activation_time' ] );
			add_action( 'admin_init', [ __CLASS__, 'rtwpvg_check_installation_time' ] );
			add_action( 'admin_init', [ __CLASS__, 'rtwpvg_spare_me' ], 5 );
		}
	}
