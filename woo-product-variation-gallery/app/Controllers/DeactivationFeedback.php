...
class DeactivationFeedback {
	 ...
	 * @var string
	 */
	private const PREFIX = 'rtwpvg-deactivation';

	/**
	 * Plugin identifier expected by the survey endpoint.
	 *
	 * Also selects the spreadsheet tab the row is appended to, so it must
	 * stay exactly "VariationGallery".
	 *
	 * @var string
	 */
	private const PLUGIN_KEY = 'VariationGallery';
	 ...
	 * @var string
	 */
	private const SURVEY_ENDPOINT = 'https://shopbuilderwp.com/wp-json/RadiusTheme/pluginSurvey/v1/Survey/appendToSheet';
	 ...
	 * @var string
	 */
	private const SUPPORT_URL = 'https://www.radiustheme.com/contact/';
	...
	public static function get_reasons() {
		return [
			'bug_issue_detected'     => esc_html__( 'Bug or issue detected', 'woo-product-variation-gallery' ),
			'theme_conflict'         => esc_html__( 'Conflicts with my theme or another plugin', 'woo-product-variation-gallery' ),
			'found_a_better_plugin'  => esc_html__( 'I found a better plugin', 'woo-product-variation-gallery' ),
			'no_longer_needed'       => esc_html__( 'I no longer need the plugin', 'woo-product-variation-gallery' ),
			'temporary_deactivation' => esc_html__( "It's a temporary deactivation", 'woo-product-variation-gallery' ),
		];
	}
	 ...
	 *
	 * `get_current_screen()` is deliberately not used here: the dialog has to
	 * render even when the screen object is unavailable or filtered away.
	 *
	 * @return void
	 */
	...
	public static function render_markup() {
		$prefix = self::PREFIX;
		?>
		<div id="<?php echo esc_attr( $prefix ); ?>-dialog" title="<?php esc_attr_e( 'Quick Feedback', 'woo-product-variation-gallery' ); ?>">
			<div class="<?php echo esc_attr( $prefix ); ?>-content">
				<div id="<?php echo esc_attr( $prefix ); ?>-form-body">
					<p class="<?php echo esc_attr( $prefix ); ?>-intro">
						<?php esc_html_e( 'Having trouble?', 'woo-product-variation-gallery' ); ?><br/>
						<?php
						printf(
							/* translators: %s: Support ticket link. */
							esc_html__( 'For faster and more accurate support, please %s — our support agent will personally review and resolve your issue.', 'woo-product-variation-gallery' ),
							sprintf(
								'<a target="_blank" href="%1$s" class="%2$s-support-link">%3$s</a>',
								esc_url( self::SUPPORT_URL ),
								esc_attr( $prefix ),
								esc_html__( 'open a support ticket', 'woo-product-variation-gallery' )
							)
						);
						?>
					</p>
					...
					<div class="<?php echo esc_attr( $prefix ); ?>-input-header">
						<?php esc_html_e( 'If you’d prefer not to open a support ticket, please take a moment to share why you’re deactivating Variation Images Gallery for WooCommerce:', 'woo-product-variation-gallery' ); ?>
					</div>
						...
						<div class="<?php echo esc_attr( $prefix ); ?>-input-wrapper<?php echo 'found_a_better_plugin' === $reason_key ? ' conditional' : ''; ?>">
							...
							<?php if ( 'found_a_better_plugin' === $reason_key ) : ?>
								<input class="<?php echo esc_attr( $prefix ); ?>-better-plugin"
									   type="text"
									   name="rtwpvg_reason_found_a_better_plugin"
									   placeholder="<?php esc_attr_e( 'Please share the plugin name', 'woo-product-variation-gallery' ); ?>">
							<?php endif; ?>
						</div>
					<?php endforeach; ?>
				...
				<div class="<?php echo esc_attr( $prefix ); ?>-extra" style="display:none;">
					<p>
						<?php esc_html_e( 'Please let us know about any issues you are facing with the plugin. How can we improve the plugin?', 'woo-product-variation-gallery' ); ?>
					</p>
					<div class="<?php echo esc_attr( $prefix ); ?>-text-wrapper">
						<textarea id="<?php echo esc_attr( $prefix ); ?>-feedback" rows="2" cols="40"
								  placeholder="<?php esc_attr_e( 'Write something here. How can we improve the plugin?', 'woo-product-variation-gallery' ); ?>"></textarea>
						<span class="<?php echo esc_attr( $prefix ); ?>-error"></span>
					</div>
				</div>
			</div>
	 ...
	 * a slug. The handler is delegated and row scoped, so no other plugin's
	 * Deactivate link is ever intercepted.
	 *
	 * @return void
	 */
	public static function render_scripts() {
		...
		<script id="<?php echo esc_attr( $prefix ); ?>-script">
			jQuery( function ( $ ) {
				...
				var pluginFile   = <?php echo self::js_string( RTWPVG_PLUGIN_BASENAME ); ?>;
				...
				function submitFeedback() {
					...
					if ( ! reasons ) {
						$( '#' + prefix + '-form-body' ).children( '.' + prefix + '-error' )
							.text( <?php echo self::js_string( __( 'Choose the reason', 'woo-product-variation-gallery' ) ); ?> );
						return;
					}

					if ( 'bug_issue_detected' === reasons && ! feedback ) {
						$dialog.find( '.' + prefix + '-text-wrapper .' + prefix + '-error' )
							.text( <?php echo self::js_string( __( 'Please provide more details regarding the issue so we can address it in future updates.', 'woo-product-variation-gallery' ) ); ?> );
						return;
					}

					// The survey endpoint discards rows with an empty feedback
					// field, so a plugin name on its own would never reach the
					// sheet. Promote it to the feedback text instead.
					if ( ! feedback && 'found_a_better_plugin' === reasons && betterPlugin ) {
						feedback = 'Switched to: ' + betterPlugin;
					}
					...
					//
					// The survey API builds the spreadsheet row from the order
					// the parameters arrive in, so this object's key order maps
					// to columns A-G and must not be changed.
					$.ajax( {
						...
						data: {
							...
							better_plugin: 'found_a_better_plugin' === reasons ? betterPlugin : '',
							feedback: feedback,
							wpplugin: <?php echo self::js_string( self::PLUGIN_KEY ); ?>,
							version: <?php echo self::js_string( defined( 'RTWPVG_VERSION' ) ? RTWPVG_VERSION : '' ); ?>,
				...
				$( document ).on( 'click', linkSelector, function ( e ) {
					...
					$dialog.dialog( {
						...
						buttons: [
							{
								text: <?php echo self::js_string( __( 'Submit & Deactivate', 'woo-product-variation-gallery' ) ); ?>,
								click: submitFeedback
							},
							{
								text: <?php echo self::js_string( __( 'Skip & Deactivate', 'woo-product-variation-gallery' ) ); ?>,
								click: deactivate
							}
						]
					} );
