(() => {//#region src/js/meta-boxes-product-variation.js
jQuery(function($) {
	"use strict";
	/**
	* Variations actions
	...
	var wc_meta_boxes_product_variations_actions = {
		...
		*/
		init: function() {
			...
			$(document.body).on("woocommerce_variations_added", this.variation_added).on("keyup", ".wc_input_variations_price", this.maybe_enable_button_to_add_price_to_variations);
		},
		create_variations: function(event) {
			...
			$.post(woocommerce_admin_meta_boxes.ajax_url, data, function(response) {
				if (response.error) {
					...
					this_page_url = this_page_url.replace("post-new.php?", "post.php?post=" + woocommerce_admin_meta_boxes.post_id + "&action=edit&");
					$.get(this_page_url, function(response) {
						$("#variable_product_options").unblock();
						$("#variable_product_options_inner").replaceWith($(response).find("#variable_product_options_inner"));
						$("#variable_product_options").trigger("reload");
						$("#product_attributes > .product_attributes").replaceWith($(response).find("#product_attributes > .product_attributes"));
					});
				}
			});
		},
		...
		*/
		reload: function() {
			wc_meta_boxes_product_variations_ajax.load_variations(1).then(wc_meta_boxes_product_variations_ajax.show_hide_variation_empty_state);
		...
		*/
		variable_is_downloadable: function() {
			...
			if ($(this).is(":checked")) $(this).closest(".woocommerce_variation").find(".show_if_variation_downloadable").show();
		},
		/**
		...
		*/
		variable_is_virtual: function() {
			...
			if ($(this).is(":checked")) $(this).closest(".woocommerce_variation").find(".hide_if_variation_virtual").hide();
		},
		/**
		...
		*/
		maybe_enable_button_to_add_price_to_variations: function() {
			...
			if (isNaN(variation_price)) $(".add_variations_price_button").prop("disabled", true);
			else $(".add_variations_price_button").prop("disabled", false);
		},
		/**
		...
		*/
		variable_manage_stock: function() {
			...
			if ($("input#_manage_stock:checked").length) $(this).closest(".woocommerce_variation").find(".variable_stock_status").hide();
		},
		/**
		* Notice dismiss
		*/
		...
		variations_loaded: function(event, needsUpdate) {
			...
			if (!needsUpdate) {
				...
				$(".woocommerce_variation", wrapper).each(function(index, el) {
					var $el = $(el), date_from = $(".sale_price_dates_from", $el).val(), date_to = $(".sale_price_dates_to", $el).val();
					if ("" !== date_from || "" !== date_to) $("a.sale_schedule", $el).trigger("click");
				});
			...
			$(".woocommerce_variations .tips, .woocommerce_variations .help_tip, .woocommerce_variations .woocommerce-help-tip, .toolbar-variations-defaults .woocommerce-help-tip", wrapper).tipTip({
				attribute: "data-tip",
				fadeIn: 50,
				fadeOut: 50,
				delay: 200
			...
			$(".sale_price_dates_fields", wrapper).find("input").datepicker({
				...
				onSelect: function() {
					var option = $(this).is(".sale_price_dates_from") ? "minDate" : "maxDate", dates = $(this).closest(".sale_price_dates_fields").find("input"), date = $(this).datepicker("getDate");
		...
		*/
		variation_added: function(event, qty) {
			if (1 === qty) wc_meta_boxes_product_variations_actions.variations_loaded(null, true);
		},
		/**
		...
		*/
		set_variations_price: function() {
			...
			$(`.wc_input_price[name^=${"variable-subscription" === $("select#product-type").val() ? "variable_subscription_sign_up_fee" : "variable_regular_price"}]`).each(function(index, el) {
				if ("0" === $(el).val() || "" === $(el).val()) $(el).val(variation_price).trigger("change");
			});
		...
		*/
		open_modal_to_set_variations_price: function() {
			$(this).WCBackboneModal({ template: "wc-modal-set-price-variations" });
			$(".add_variations_price_button").on("click", wc_meta_boxes_product_variations_actions.set_variations_price);
		},
		/**
		...
		*/
		set_menu_order: function(event) {
			...
			if (value != null) {
				...
				$(this).closest(".woocommerce_variation").append("<input type=\"hidden\" name=\"new_variation_menu_order_id\" value=\"" + encodeURIComponent(variation_id) + "\" />");
				$(this).closest(".woocommerce_variation").append("<input type=\"hidden\" name=\"new_variation_menu_order_value\" value=\"" + encodeURIComponent(parseInt(value, 10)) + "\" />");
				wc_meta_boxes_product_variations_ajax.save_variations();
			}
		},
		/**
		...
		*/
		variation_row_indexes: function() {
			var wrapper = $("#variable_product_options").find(".woocommerce_variations"), current_page = parseInt(wrapper.attr("data-page"), 10), offset = parseInt((current_page - 1) * woocommerce_admin_meta_boxes_variations.variations_per_page, 10);
			$(".woocommerce_variations .woocommerce_variation").each(function(index, el) {
				$(".variation_menu_order", el).val(parseInt($(el).index(".woocommerce_variations .woocommerce_variation"), 10) + 1 + offset).trigger("change");
			});
		}
	};
	/**
	...
	var wc_meta_boxes_product_variations_media = {
		...
		*/
		add_image: function(event) {
			var $button = $(this), post_id = $button.attr("rel");
			wc_meta_boxes_product_variations_media.setting_variation_image = $button.closest(".upload_image");
			...
			if ($button.is(".remove")) {
				...
				if (wc_meta_boxes_product_variations_media.variable_image_frame) {
					...
					return;
				} else wp.media.model.settings.post.id = wc_meta_boxes_product_variations_media.setting_variation_image_id;
				wc_meta_boxes_product_variations_media.variable_image_frame = wp.media.frames.variable_image = wp.media({
					...
					button: { text: woocommerce_admin_meta_boxes_variations.i18n_set_image },
				...
				wc_meta_boxes_product_variations_media.variable_image_frame.on("select", function() {
					var attachment = wc_meta_boxes_product_variations_media.variable_image_frame.state().get("selection").first().toJSON(), url = attachment.sizes && attachment.sizes.thumbnail ? attachment.sizes.thumbnail.url : attachment.url;
	...
	var wc_meta_boxes_product_variations_ajax = {
		...
		*/
		init: function() {
			...
			$(document.body).on("change input", "#variable_product_options .woocommerce_variations :input", this.input_changed).on("change", ".variations-defaults select", this.defaults_changed);
			var postForm = $("form#post");
			postForm.on("submit", this.save_on_submit);
			$("input:submit", postForm).on("click keypress", function() {
				postForm.data("callerid", this.id);
			});
			$(".wc-metaboxes-wrapper").on("change", "#field_to_edit", this.do_variation_action);
			$(".wc-metaboxes-wrapper").on("click", "button.generate_variations", this.generate_variations);
			$(".wc-metaboxes-wrapper").on("click", "button.add_variation_manually", this.add_variation_manually);
		},
		/**
		...
		*/
		check_for_changes: function() {
			...
			if (0 < need_update.length) {
				if (window.confirm(woocommerce_admin_meta_boxes_variations.i18n_edited_variations)) wc_meta_boxes_product_variations_ajax.save_changes();
				else {
		...
		*/
		initial_load: function() {
			if (0 === $("#variable_product_options").find(".woocommerce_variations .woocommerce_variation").length) wc_meta_boxes_product_variations_pagenav.go_to_page();
		},
		/**
		...
		*/
		save_changes: function(callback) {
			var need_update = $(".variation-needs-update", $("#variable_product_options").find(".woocommerce_variations")), data = {};
			if (0 < need_update.length) {
				wc_meta_boxes_product_variations_ajax.block();
		...
		*/
		save_variations: function() {
			...
			wc_meta_boxes_product_variations_ajax.save_changes(function(error) {
				var wrapper = $("#variable_product_options").find(".woocommerce_variations"), current = wrapper.attr("data-page");
				$("#variable_product_options").find("#woocommerce_errors").remove();
				if (error) wrapper.before(error);
		...
		*/
		save_on_submit: function(e) {
			if (0 < $("#variable_product_options").find(".woocommerce_variations .variation-needs-update").length) {
				e.preventDefault();
		...
		*/
		save_on_submit_done: function() {
			var postForm = $("form#post");
			if ("publish" === postForm.data("callerid")) postForm.append("<input type=\"hidden\" name=\"publish\" value=\"1\" />").trigger("submit");
		...
		*/
		remove_variation: function() {
			...
			if (window.confirm(woocommerce_admin_meta_boxes_variations.i18n_remove_variation)) {
				var variation = $(this).attr("rel"), variation_ids = [], data = { action: "woocommerce_remove_variations" };
				wc_meta_boxes_product_variations_ajax.block();
				if (0 < variation) {
					...
					$.post(woocommerce_admin_meta_boxes_variations.ajax_url, data, function() {
						var wrapper = $("#variable_product_options").find(".woocommerce_variations"), current_page = parseInt(wrapper.attr("data-page"), 10), total_pages = Math.ceil((parseInt(wrapper.attr("data-total"), 10) - 1) / woocommerce_admin_meta_boxes_variations.variations_per_page), page = 1;
						$("#woocommerce-product-data").trigger("woocommerce_variations_removed");
						if (current_page === total_pages || current_page <= total_pages) page = current_page;
						else if (current_page > total_pages && 0 !== total_pages) page = total_pages;
						wc_meta_boxes_product_variations_pagenav.go_to_page(page, -1);
					});
				} else wc_meta_boxes_product_variations_ajax.unblock();
				window.wcTracks.recordEvent("product_variations_buttons", { action: "remove_variation" });
			}
			return false;
		},
		/**
		...
		link_all_variations: function() {
			...
			if (window.confirm(woocommerce_admin_meta_boxes_variations.i18n_link_all_variations)) {
				...
				$.post(woocommerce_admin_meta_boxes_variations.ajax_url, data, function(response) {
					...
					if (1 === count) window.alert(count + " " + woocommerce_admin_meta_boxes_variations.i18n_variation_added);
					else if (0 === count || count > 1) window.alert(count + " " + woocommerce_admin_meta_boxes_variations.i18n_variations_added);
					else window.alert(woocommerce_admin_meta_boxes_variations.i18n_no_variations_added);
					wc_meta_boxes_product_variations_ajax.show_hide_variation_empty_state();
					if (count > 0) {
				...
				window.wcTracks.recordEvent("product_variations_buttons", { action: "generate_variations" });
			}
			return false;
		},
		/**
		...
		*/
		input_changed: function(event) {
			...
			$("button.cancel-variation-changes, button.save-variation-changes").prop("disabled", false);
			if ("input" === event.type && $(this).is(":text")) return;
		...
		*/
		do_variation_action: function() {
			var do_variation_action = $(this).val(), data = {}, changes = 0, value;
			switch (do_variation_action) {
				...
				case "variable_sale_price_decrease":
					...
					if (value != null) {
						if (value.indexOf("%") >= 0) data.value = accounting.unformat(value.replace(/\%/, ""), woocommerce_admin.mon_decimal_point) + "%";
						else data.value = accounting.unformat(value, woocommerce_admin.mon_decimal_point);
					} else return;
					break;
				...
				case "variable_sale_schedule":
					...
					if (null === data.date_from) data.date_from = false;
					if (null === data.date_to) data.date_to = false;
					if (false === data.date_to && false === data.date_from) return;
					break;
				default:
			...
			if ("delete_all" === do_variation_action && data.allowed) $("#variable_product_options").find(".variation-needs-update").removeClass("variation-needs-update");
			else wc_meta_boxes_product_variations_ajax.check_for_changes();
			wc_meta_boxes_product_variations_ajax.block();
			$.ajax({
				url: woocommerce_admin_meta_boxes_variations.ajax_url,
				data: {
	...
	var wc_meta_boxes_product_variations_pagenav = {
		...
		*/
		update_variations_count: function(qty) {
			var wrapper = $("#variable_product_options").find(".woocommerce_variations"), total = parseInt(wrapper.attr("data-total"), 10) + qty, displaying_num = $(".variations-pagenav .displaying-num");
			wrapper.attr("data-total", total);
			if (1 === total) displaying_num.text(woocommerce_admin_meta_boxes_variations.i18n_variation_count_single.replace("%qty%", total));
			else displaying_num.text(woocommerce_admin_meta_boxes_variations.i18n_variation_count_plural.replace("%qty%", total));
			return total;
		},
		/**
		...
		*/
		set_paginav: function set_paginav(qty) {
			var wrapper = $("#variable_product_options").find(".woocommerce_variations"), new_qty = wc_meta_boxes_product_variations_pagenav.update_variations_count(qty), toolbar = $("#variable_product_options").find(".toolbar"), variation_action = $(".variation_actions"), page_nav = $(".variations-pagenav"), displaying_links = $(".pagination-links", page_nav), total_pages = Math.ceil(new_qty / woocommerce_admin_meta_boxes_variations.variations_per_page), options = "";
			wrapper.attr("data-total_pages", total_pages);
			$(".total-pages", page_nav).text(total_pages);
			for (var i = 1; i <= total_pages; i++) options += "<option value=\"" + i + "\">" + i + "</option>";
			...
			if (0 === new_qty) {
				...
				$(".variation_actions").val("add_variation");
				if (1 === total_pages) displaying_links.hide();
				else displaying_links.show();
			}
		},
		/**
		...
		*/
		change_classes: function(selected, total) {
			var first_page = $(".variations-pagenav .first-page"), prev_page = $(".variations-pagenav .prev-page"), next_page = $(".variations-pagenav .next-page"), last_page = $(".variations-pagenav .last-page");
			if (1 === selected) {
		...
		*/
		page_selector: function() {
			var selected = parseInt($(this).val(), 10), wrapper = $("#variable_product_options").find(".woocommerce_variations");
			...
			wc_meta_boxes_product_variations_ajax.load_variations(selected).then(wc_meta_boxes_product_variations_ajax.show_hide_variation_empty_state());
		},
		/**
		* Go to first page
		*
		...
		*/
		first_page: function() {
			if (wc_meta_boxes_product_variations_pagenav.check_is_enabled(this)) wc_meta_boxes_product_variations_pagenav.set_page(1);
			return false;
		},
		/**
		* Go to previous page
		...
		*/
		prev_page: function() {
			if (wc_meta_boxes_product_variations_pagenav.check_is_enabled(this)) {
				var wrapper = $("#variable_product_options").find(".woocommerce_variations"), prev_page = parseInt(wrapper.attr("data-page"), 10) - 1, new_page = 0 < prev_page ? prev_page : 1;
		...
		*/
		next_page: function() {
			if (wc_meta_boxes_product_variations_pagenav.check_is_enabled(this)) {
				var wrapper = $("#variable_product_options").find(".woocommerce_variations"), total_pages = parseInt(wrapper.attr("data-total_pages"), 10), next_page = parseInt(wrapper.attr("data-page"), 10) + 1, new_page = total_pages >= next_page ? next_page : total_pages;
				wc_meta_boxes_product_variations_pagenav.set_page(new_page);
			}
			return false;
		},
